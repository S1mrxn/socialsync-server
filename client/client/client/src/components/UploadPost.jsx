...
export default function UploadPost({ onUpload }) {
  ...
  return (
    <form
      ...
      className="bg-[#0f172a] text-white border border-gray-700 rounded-xl p-6 mb-6 shadow-md"
    >
      <h3 className="text-lg font-semibold mb-4">Create a Post</h3>

      {/* Image Upload */}
      <label className="block text-sm mb-2 text-gray-300">Choose an image</label>
      <input
        type="file"
        accept="image/*"
        onChange={handleFileChange}
        className="block w-full text-sm text-gray-300 file:mr-4 file:py-2 file:px-4 file:border-0 file:rounded file:bg-secondary file:text-white hover:file:bg-secondary/90 mb-4 cursor-pointer"
      />
      ...
      {/* Caption Input */}
      <label className="block text-sm mb-1 text-gray-300">Caption</label>
      <input
        type="text"
